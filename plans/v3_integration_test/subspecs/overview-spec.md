...
10. **One commit per sub-spec.** Each `subspecNN-…` file in this
...
- The **C1 fix** — `m_lastTransition` survives an `extractData()` NotYet
  path at `SNSLiveEventDataListener.cpp:1515-1516`.
- The **deferred-run-details invariant** at
  `SNSLiveEventDataListener.cpp:1562-1568` — coverage delegated to the
  existing no-network unit suite (see `subspec06`).
- The white-lie path on mid-run join at
  `SNSLiveEventDataListener.cpp:654-697`.
...
4. `Framework/LiveData/test/CMakeLists.txt` lists `MockSMSServer.cpp` in
   ...
   (`subspec03`).
7. No file under `Framework/LiveData/src/` or `Framework/LiveData/inc/`
