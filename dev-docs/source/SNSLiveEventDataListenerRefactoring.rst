...
.. code-block:: cpp
   ...
   SNSLiveEventDataListener::extractData() {     // conceptually
       ...
       {
           // Throw NotYet immediately if the background thread is mid-parse.
...
and ``LiveListener::extractData()`` invokes ``onAfterExtract()``
**only** when ``doExtractData()`` returns normally.  An
``Exception::NotYet`` thrown from Phase 2/3 therefore leaves any
previously memoised ``m_lastTransition`` intact, so that
``LoadLiveData``'s ``NotYet`` retry loop
(``LoadLiveData.cpp:476–490``) does not lose the edge across
re-invocation.  This is the **C1 invariant**.
...
* **``m_lastTransition`` is cleared only on the success path of
  ``doExtractData()`` — i.e. only inside ``onAfterExtract()``** —
  because the C1 invariant requires the edge
  to survive ``Exception::NotYet`` retries from ``LoadLiveData``.  From this
  single rule the ``BeginRun``-vs-``EndRun`` delivery asymmetry
  follows automatically: ``BeginRun`` is set inside
  ``onBeforeExtract()`` and cleared in ``onAfterExtract()`` on the
  same successful call (the caller already received the new run via
  the workspace itself — fresh run number, fresh buffer); ``EndRun``
  is set inside ``onAfterExtract()`` *after* the unconditional clear,
  so the caller sees ``lastTransition() == EndRun`` after the call
  returns and the next call sees ``nullopt``.
...
* **Exception-safe.**  Phase 4 (``onAfterExtract()``) is the only
  place where ``m_lastTransition`` is cleared.  Because
  ``LiveListener::extractData()`` calls ``onAfterExtract()`` only
  after ``doExtractData()`` returns normally, any
  ``Exception::NotYet`` thrown from Phase 2/3 structurally bypasses
  the clear — the :ref:`C1 invariant <sns-c1-invariant>`
  (``m_lastTransition`` survives ``LoadLiveData``'s retry loop) is a
  consequence of the template-method contract, not an extra branch.  A pending
