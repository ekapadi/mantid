...
back-pressure is needed (there is no prior run's events to protect).
...
* **``m_lastTransition`` is cleared only on the success path of
  ...
  because the C1 invariant requires the edge to survive
  ``Exception::NotYet`` retries from ``LoadLiveData``.  From this
...
* **Exception-safe.**  Phase 4 (``onAfterExtract()``) is the only
  ...
  the clear — the C1 invariant (``m_lastTransition`` survives
  ``LoadLiveData``'s retry loop) is a consequence of the
  template-method contract, not an extra branch.  A pending
...
required a live SEQUOIA instrument connection.  This file is
**intentionally not registered** in
``Framework/LiveData/CMakeLists.txt`` and therefore never runs in CI.
It is retained as a historical reference only; the integration and
unit test files above supersede it.
