...
3. **Exception safety across the three-phase split.**  If
   ...
   assignment — are preserved across the retry.  Placing
   edge-clearing in ``onAfterExtract()`` rather than at the top of
   the next ``onBeforeExtract()`` call means the edge automatically
   survives any number of ``NotYet`` retries without requiring a
   separate cross-call flag.
...
``m_lastTransition`` is cleared **only** inside ``onAfterExtract()``,
and the ``LiveListener::extractData()`` template method invokes
``onAfterExtract()`` **only** when ``doExtractData()`` returns
normally.  A ``doExtractData()`` call that throws
``Exception::NotYet`` therefore leaves any previously memoised
``m_lastTransition`` intact, so that ``LoadLiveData``'s ``NotYet``
retry loop (``LoadLiveData.cpp:476–490``) does not lose the edge
across re-invocation.  This is the **C1 invariant**.

Two subtleties to flag in code review
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

1. **``m_lastTransition`` lifetime.**  The only safe place to clear
   ``m_lastTransition`` is after a successful ``doExtractData()`` —
   i.e., inside ``onAfterExtract()`` — because ``doExtractData()``
   may throw ``Exception::NotYet``, and the
   :ref:`C1 invariant <migration-c1-invariant>` requires the
   edge to survive the ``LoadLiveData`` retry loop.  The observable
   consequence is a ``BeginRun`` / ``EndRun`` asymmetry:
   ``BeginRun`` is set in ``onBeforeExtract()`` and cleared in
   ``onAfterExtract()``, so a caller who queries ``lastTransition()``
   *after* a successful ``extractData()`` sees ``nullopt`` — the
   ``BeginRun`` was already delivered via the workspace (new run
   number, fresh buffer).  ``EndRun`` is set in ``onAfterExtract()``
   after the buffer has been harvested, so ``lastTransition()``
   returns ``EndRun`` until the *next* ``extractData()`` call's
   ``onAfterExtract()`` clears it.  A caller that never calls
   ``extractData()`` never sees an edge (no commit, no edge).
...
``Exception::NotYet``.  The classic example is clearing a
"transition-was-delivered" edge: if the clear ran unconditionally in
``onBeforeExtract()``, a ``NotYet`` retry would lose the edge even
though the workspace was never handed to the caller.
