...
1. **``m_lastTransition`` lifetime.**  The only safe place to clear
   ...
   may throw ``Exception::NotYet``, and the C1 invariant requires the
...
``onAfterExtract()`` fires only on the success path, so the C1
invariant (edge survives retries) is a direct structural consequence
of using it rather than an inductive argument over a separate
cross-call flag.  No in-tree listener other than
